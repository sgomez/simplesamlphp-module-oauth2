...
class sspmod_oauth2_Registry
{
	...
	public function formToMeta($request, $entry = array(), $override = NULL) {
		...
		$this->getStandardField($request, $entry, 'redirect_uri', true);
		$this->getStandardField($request, $entry, 'allowed_origins', true);
		$this->getStandardField($request, $entry, 'client_id');
	...
	public function metaToForm($metadata) {
		...
		return '<form action="registry.edit.php" method="post">' .		
			...
			'<div id="basic"><table class="formtable">' .
				...
				$this->arrayField($metadata, 'redirect_uri', 'Static/enforcing callback-url (one per line)', TRUE) .
				$this->arrayField($metadata, 'allowed_origins', 'Allowed Javascript domains (one per line)', TRUE) .
				
				$this->hiddenField('field_client_id', $metadata['client_id']) .
			'</table></div>' . 
			'</div>' .
